...
import { v4 } from "uuid";
import { cleanUuid, mapEntities, NULL_UUID } from "./entities";
...
describe(__filename, () => {
  describe("mapEntitiesFrom()", () => {
    ...
    it("return undefined for null/undefined inputs", () => {
      expect(mapEntities(spy, null)).to.equal(undefined);
      expect(mapEntities(spy, undefined)).to.equal(undefined);
      spy.notCalled.should.equal(true);
    });

    it("return an empty array for empty array inputs", () => {
      mapEntities(spy, []).should.deep.equal([]);
      spy.notCalled.should.equal(true);
    });

    it("should call the mapper for each value in the input array", () => {
      mapEntities(spy, ["val1", "val2"]);
