...
describe(__filename, () => {
  ...
  it("should render a text label", () => {
    mountWrapper = mount(
      <ControlledSelectInput id={"test-id"} value={"test-val"} label={"Test Label"} onValueChange={voidListener} />,
    );
  ...
  it("should call the change listener when the value changes", () => {
    const spy = sinon.spy();
    mountWrapper = mount(
      <ControlledSelectInput id={"test-id"} value={"test-val"} label={"Test Label"} onValueChange={spy} />,
    );
