...
describe(__filename, () => {
	...
	it("should pass core props to the input", () => {
		...
		mountWrapper.find("textarea").props().id.should.equal("test-id");
		mountWrapper.find("textarea").props().value.should.equal("test-val");
		mountWrapper.find("textarea").props().placeholder.should.equal("test-placeholder");
		mountWrapper.find("textarea").props().disabled.should.equal(false);
		...
		mountWrapper.find("textarea").props().id.should.equal("test-id2");
		mountWrapper.find("textarea").props().value.should.equal("test-val2");
		mountWrapper.find("textarea").props().placeholder.should.equal("test-placeholder2");
		mountWrapper.find("textarea").props().disabled.should.equal(true);
	});
	...
	it("should not render an error if none is provided", () => {
		...
		mountWrapper.find("textarea").props().className.should.not.contain(bs.isInvalid);
	...
	it("should render an error if one is provided and the component has been touched", () => {
		...
		mountWrapper.update();
		mountWrapper.find("textarea").props().className.should.contain(bs.isInvalid);
